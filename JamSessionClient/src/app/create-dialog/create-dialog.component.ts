...
export class CreateDialogComponent extends HttpClientService implements OnInit {
  ...
  constructor(private http2: HttpClient,
              public dialogRef: MatDialogRef<CreateDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: any) {
    super(http2);
    if (data.onlyJoin) {
    ...
    if (this.nameFormControl.status === 'VALID' && (this.sessionNameFormControl.status === 'VALID' || this.onlyJoin) &&
     this.passwordFormControl.status === 'VALID' && this.instrumentFormControl.status === 'VALID') {
      ...
      if (this.instrumentFormControl.value === 'DRUM') {
        if (this.snareEffectFormControl.status === 'VALID' &&
        this.tomEffectFormControl.status === 'VALID' && this.hithatEffectFormControl.status === 'VALID'
  ...
  private sessionWithDrum() {
    const data = {user: this.nameFormControl.value, client: this.client,
      sessionId: this.sessionId, password: this.passwordFormControl.value, sounds: [
  ...
  private sessionNoDrum() {
    ...
    const data = {user: this.nameFormControl.value, client: this.client,
      sessionId: this.sessionId, password: this.passwordFormControl.value, octave: octave1, sounds: [
